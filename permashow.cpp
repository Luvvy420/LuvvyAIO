...
void Permashow_MenuElementChange(TreeEntry* element) {
    ...
    case TreeEntryType::Hotkey:
        ...
        if (element->element_type() == TreeEntryType::Hotkey) {
            ...
            switch (virtual_key) {
            case 0: output_text.append("None"); break;
            case 4: output_text.append("MB3"); break;
            case 5: output_text.append("MB4"); break;
            case 6: output_text.append("MB5"); break;
            case 7: output_text.append("None"); break;
            case 112: output_text.append("F1"); break;
            case 113: output_text.append("F2"); break;
            case 114: output_text.append("F3"); break;
            case 115: output_text.append("F4"); break;
            case 116: output_text.append("F5"); break;
            case 117: output_text.append("F6"); break;
            case 118: output_text.append("F7"); break;
            case 119: output_text.append("F8"); break;
            case 120: output_text.append("F9"); break;
            case 121: output_text.append("F10"); break;
            case 122: output_text.append("F11"); break;
            case 123: output_text.append("F12"); break;
            default:
        ...
        // Luvvy style: Use blue for enabled, pink for disabled
        it->Value = output_text;
        it->ValueColor = element->get_bool() ? MAKE_COLOR(0, 222, 255, 255) : MAKE_COLOR(230, 39, 255, 200);
        break;
    }
...
void Permashow::Init(TreeTab* main_tab, std::string title) {
    Initialized = true;
    permashow = main_tab->add_tab("luvvy.permashow", "Permashow (Luvvy)");
    {
        permashow_enabled = permashow->add_checkbox("luvvy.permashow.enabled", "Enabled", true);

        permashow_pos_x = permashow->add_slider("luvvy.permashow.posx", "Position X", int(renderer->screen_width() * 0.75f), 0, renderer->screen_width());
        permashow_pos_y = permashow->add_slider("luvvy.permashow.posy", "Position Y", int(renderer->screen_height() * 0.77f), 0, renderer->screen_height());

        permashow_fontsize = permashow->add_slider("luvvy.permashow.font", "Font Size", 16, 11, 26);
        permashow_fontsize->add_property_change_callback(Permashow_FontChange);

        auto colors = permashow->add_tab("luvvy.permashow.colors", "Colors & Style");
        {
            // BLACK + BROWN NIGHT STYLE
            background_color = colors->add_colorpick("luvvy.permashow.background_color", "Background", { 15.f / 255.f, 12.f / 255.f, 8.f / 255.f, 245.f / 255.f }); // deep black-brown
            text_color = colors->add_colorpick("luvvy.permashow.text_color", "Text", { 230.f / 255.f, 210.f / 255.f, 185.f / 255.f, 255.f / 255.f }); // warm beige
            border_color = colors->add_colorpick("luvvy.permashow.border_color", "Border", { 60.f / 255.f, 40.f / 255.f, 18.f / 255.f, 220.f / 255.f }); // rich brown
            separator_color = colors->add_colorpick("luvvy.permashow.separator_color", "Separator", { 80.f / 255.f, 55.f / 255.f, 32.f / 255.f, 150.f / 255.f }); // lighter brown
            title_color = colors->add_colorpick("luvvy.permashow.title_color", "Title", { 255.f / 255.f, 212.f / 255.f, 143.f / 255.f, 255.f / 255.f }); // warm amber/gold
            title_background_color = colors->add_colorpick("luvvy.permashow.title_background_color", "Title Background", { 34.f / 255.f, 24.f / 255.f, 14.f / 255.f, 240.f / 255.f }); // dark brown
        }
    }
